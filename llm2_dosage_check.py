...
PERSIST_DIR = "./bnf_storage"
BNF_PDF_PATH = "/home/manogyaguragai/Desktop/Projects/discharge-doc/BNF 84.pdf"

def initialize_bnf_bm25(bnf_pdf_path=BNF_PDF_PATH):
    """
    Initialize or load a BM25 retriever using a persisted BNF vector store.
    If the vector store does not exist, create it from the provided BNF PDF and persist it.
    """
    ...
    if os.path.exists(PERSIST_DIR) and os.listdir(PERSIST_DIR):
        try:
            ...
            print("Loaded persisted BNF vector store.")
        ...
        try:
            ...
            print("Created and persisted new BNF vector store.")
        except Exception as e:
            print(f"Error creating BNF vector store: {e}")
            sys.exit(1)
...
def cross_reference_dosage(medication, bm25_retriever):
    ...
    to fetch relevant BNF nodes and use an LLM to extract the recommended dosage.
    
    The returned JSON object includes:
      - name
      - prescribed_dosage (dosage from above)
      - bnf_dosage
      - correctness (1 if the prescribed dosage exactly matches the BNF dosage, otherwise 0)
    ...
    prompt = f"""
        ...
        Given the medication details below and the BNF reference information, determine the recommended dosage.

        Medication:
        - Name: {medication['name']}
        - Prescribed dosage: {prescribed_dosage}

        BNF Reference Information:
        \"\"\"
        {context}
        \"\"\"

        From the BNF reference, extract the recommended dosage for the medication and compare it with the prescribed dosage.
        Return a JSON object with the following keys:
        - name: medication name
        - prescribed_dosage: as provided above
        - bnf_dosage: the dosage extracted from the BNF reference
        - correctness: 1 if the prescribed dosage exactly matches the BNF dosage, otherwise 0
        - remarks: a brief note explaining the comparison, if the bnf dosage is not found, mention that the given medicine name is brand.
...
def check_medications_dosage(medications):
    ...
    """
    bm25_retriever = initialize_bnf_bm25()
    final_results = []
